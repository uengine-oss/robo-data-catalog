...
class TableSampleContextBuilder:
    ...
    async def fetch(
        ...
        if not db_tables:
            log_catalog_operation("SAMPLE_CTX", "NO_TABLES", f"datasource={datasource} 에 등록 테이블 없음", logging.WARNING)
    ...
    async def _fetch_sample_rows(
        ...
        return await self._db.fetch_rows(
            session, DataFabricQueryGateway.sample_sql(table_fqn, limit), max_rows=limit
        )
