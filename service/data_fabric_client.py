...
class DataFabricClient:
    ...
    Usage:
        client = DataFabricClient(base_url="http://127.0.0.1:8004", datasource="shopmall")
    ...
    def sample_sql(table_fqn: str, limit: int) -> str:
        ...
        if "." in table_fqn:
            ...
            return f'SELECT * FROM "{schema}"."{name}" LIMIT {limit}'
        return f"SELECT * FROM {table_fqn} LIMIT {limit}"
    ...
    async def check_available(self, session: aiohttp.ClientSession) -> bool:
        ...
        try:
            ...
            log_process("FABRIC", "UNAVAILABLE", str(e)[:150], logging.DEBUG)
            return False
            ...
            try:
                async with session.post(
                    ...
                    if resp.status != 200:
                        ...
                        log_process("FABRIC", "HTTP_ERROR", f"status={resp.status}, body={body[:150]}", logging.DEBUG)
                        return None
                    ...
                    if data.get("type") == "error":
                        ...
                        if self._is_retryable(err_msg) and attempt < max_retries - 1:
                            wait = (attempt + 1) * 2
                            log_process("FABRIC", "RETRY", f"{err_msg[:100]} — {wait}초 대기 ({attempt + 1}/{max_retries})", logging.WARNING)
                            await asyncio.sleep(wait)
                            continue
                        log_process("FABRIC", "SQL_ERROR", err_msg[:150], logging.DEBUG)
                        return None
                ...
                if self._is_retryable(msg) and attempt < max_retries - 1:
                    wait = (attempt + 1) * 2
                    log_process("FABRIC", "RETRY", f"{msg[:100]} — {wait}초 대기", logging.WARNING)
                    await asyncio.sleep(wait)
                    continue
                log_process("FABRIC", "API_ERROR", msg[:150], logging.DEBUG)
