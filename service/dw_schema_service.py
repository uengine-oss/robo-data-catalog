...
async def register_star_schema(
    ...
    try:
        ...
        for col in fact_table.get("columns", []):
            ...
            col_query = {
                "query": """
                    MATCH (__cy_t__:TABLE {id:$table_fqn})
                    MERGE (__cy_t__)-[:HAS_COLUMN]->(__cy_c__:COLUMN {id:$col_fqn})
        ...
        for col in fact_table.get("columns", []):
            if col.get("is_fk") and col.get("fk_target_table"):
                ...
                fk_query = {
                    "query": """
                        MATCH (__cy_f__:TABLE {id:$fact_fqn})
                        MATCH (__cy_d__:TABLE {id:$target_fqn})
