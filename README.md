...
```text
  ...
  search/                    메타데이터 시맨틱 검색
  samples/                   테이블 샘플 context
  integrations/                  Data Fabric·embedding·LLM 연동
  shared/                    공통 인프라
    config/settings.py       타입화된 환경 설정
...
.venv\Scripts\python.exe -m compileall -q main.py api contracts enrichment external graph lineage samples search shared tests
