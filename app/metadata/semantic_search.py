...
async def search_tables_by_semantic(
    ...
    try:
        ...
        return [r for r in scored[:safe_limit] if r["similarity"] >= 0.3]
    finally:
        await client.close()
...
async def vectorize_schema_tables(
    ...
    try:
        if include_tables:
            ...
            where_conditions.append("t.graph_owner = $graph_owner")
            ...
            for table in tables:
                try:
                    ...
                    logger.error(
                        "테이블 벡터화 실패 | table=%s error_type=%s",
                        table["name"], type(e).__name__,
                    )
                    stats["errors"] += 1
