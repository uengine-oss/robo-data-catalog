...
import app.graph.graph_queries as graph_queries
...
async def test_all_user_graph_queries_apply_the_system_boundary() -> None:
    ...
    original = graph_queries.Neo4jClient
    graph_queries.Neo4jClient = client_factory  # type: ignore[assignment]
    try:
        exists = await graph_queries.check_graph_data_exists()
        payload = await graph_queries.fetch_graph_data()
    finally:
        graph_queries.Neo4jClient = original
...
async def test_check_and_delete_are_analysis_owner_scoped() -> None:
    ...
    try:
        ...
        await graph_queries.cleanup_all_graph_data(include_files=False)
    finally:
        graph_queries.Neo4jClient = original
