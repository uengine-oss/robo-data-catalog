...
confidence 점수 산식 검증 사례:
  STATUS_CODE↔STATUS_CODE (name=100, overlap=100, distinct=9) → 1.00  ✓ 확실
  TYPE_CODE↔TYPE_CODE     (name=100, overlap=100, distinct=2) → 0.82  ✓ 이름 일치 보너스
  STATUS_CODE↔STATUS_CD   (name=86,  overlap=100, distinct=9) → 0.93  ✓ 미묘한 차이도 잡음
  REGION_CODE↔ZONE_CODE   (name=80,  overlap=100, distinct=3) → 0.78  ✗ 부분일치 + 작은 distinct
  COUNT↔TAG_UNIT          (name=60,  overlap=100, distinct=2) → 0.62  ✗ 무관 컬럼
...
MATCH (c1:COLUMN)
  WHERE c1.graph_owner = $graph_owner
    AND toLower(c1.id) = toLower(item.src_schema + '.' + item.src_table + '.' + item.src_column)
MATCH (c2:COLUMN)
  WHERE c2.graph_owner = $graph_owner
    AND toLower(c2.id) = toLower(item.tgt_schema + '.' + item.tgt_table + '.' + item.tgt_column)
...
class ForeignKeyInference:
    ...
    async def _fetch_fk_candidates(
        ...
        try:
            ...
            log_catalog_operation("FK_INFER", "FETCH_ERROR", type(e).__name__, logging.WARNING)
            return None
        return rows or []
        ...
        try:
            results = await self._neo4j.execute_queries(
                [{"query": cypher, "parameters": {
                    "items": items, "graph_owner": ANALYSIS_GRAPH_OWNER,
                }}]
            )
            rows = results[0] if results else []
            return int(rows[0]["persisted"]) if rows else 0
        except Exception as e:
            log_catalog_operation("FK_INFER", "PERSIST_ERROR", type(e).__name__, logging.ERROR)
            raise
