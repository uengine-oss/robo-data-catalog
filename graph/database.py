...
class CatalogGraphDatabase:
    __slots__ = ("_driver", "_config", "_database")

    def __init__(self, database: Optional[str] = None):
        # 연결 출처: 요청 컨텍스트에 Neo4j override(Electron 헤더)가 있으면 그 값을,
        # 없으면(브라우저/테스트/CLI) Catalog graph database 설정을 쓴다.
        self._config = CATALOG_SETTINGS.graph_database
        request_connection = get_request_graph_connection()
        if request_connection is not None:
            uri, user, password = request_connection.uri, request_connection.user, request_connection.password
            default_db = request_connection.database or self._config.database
        else:
